import glob
import os
import pandas as pd

from typing import List
from scanpy import read_h5ad
from torch.utils.data import Dataset


class SingleCellDataset(Dataset):
    def __init__(
        self, h5ad_dir: str, n_files: int, n_rows_per_file: int, obs_cols: List[str], descriptions_path: str,
    ):
        self.h5ad_dir = h5ad_dir
        self.n_files = n_files
        self.n_rows_per_file = n_rows_per_file
        self.obs_cols = obs_cols
        self.files = glob.glob(os.path.join(os.getcwd()[:-3] + h5ad_dir, "*"))
        self.data = [read_h5ad(file) for file in self.files]
        self.descriptions = pd.read_csv(os.getcwd()[:-3] + descriptions_path)

    def __len__(self):
        return self.n_files * self.n_rows_per_file

    def __getitem__(self, idx: int):
        file_idx = idx // self.n_rows_per_file
        row_idx = idx % self.n_rows_per_file
        data = self.data[file_idx]
        obs = data.obs.iloc[row_idx]
        var = data.var
        x = data.X[row_idx]

        file_name = os.path.basename(self.files[file_idx])
        source_id = file_name.split(".")[0]

        matching_descriptions = self.descriptions[
            (self.descriptions['source_id'] == source_id) &
            (self.descriptions['row_id'] == row_idx)
            ]

        if len(matching_descriptions) == 0:
            return None

        return {
            'cell_data': (x, obs, var),
            'text': matching_descriptions.iloc[0]['text']
        }