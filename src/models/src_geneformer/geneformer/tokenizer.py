...
def sum_ensembl_ids(
    ...
    if file_format == "loom":
        ...
        with lp.connect(data_directory) as data:
            ...
            vals_from_map_dict = [
                gene_mapping_dict.get(gene) for gene in genes_in_map_dict
            ]
            ...
            if len(set(genes_in_map_dict)) == len(set(vals_from_map_dict)):
                mapped_vals = [
                    gene_mapping_dict.get(gene.upper())
                    for gene in data.ra["ensembl_id"]
                ]
    ...
    def tokenize_loom(self, loom_file_path, target_sum=10_000):
        ...
        with lp.connect(str(loom_file_path)) as data:
            ...
            coding_miRNA_loc = np.where(
                [
                    self.genelist_dict.get(i, False)
                    for i in data.ra["ensembl_id_collapsed"]
                ]
            )[0]
            norm_factor_vector = np.array(
                [
